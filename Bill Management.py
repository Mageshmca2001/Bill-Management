...
class Bill_App:
    def __init__(self,root):
        ...
        lbl_color = 'white'
        #Title of App
        title = Label(self.root,text = "Billing Management",bd = 12,relief = GROOVE,fg = fg_color,bg = bg_color,font=("times new roman",30,"bold"),pady = 3).pack(fill = X)
...
#Function For Text Area
    def welcome_soft(self):
        ...
        self.txt.insert(END,"       Welcome To Magesh's Retail\n")
...
#Add Product name , qty and price to bill area
    def bill_area(self):
        ...
        if self.daal.get() != 0:
            self.txt.insert(END,f"\nDaal              {self.daal.get()}           {self.daal.get() * 50}")
        if self.rice.get() != 0:
            self.txt.insert(END,f"\nRice              {self.rice.get()}           {self.rice.get() * 50}")
        if self.sugar.get() != 0:
            self.txt.insert(END,f"\nSugar             {self.sugar.get()}           {self.sugar.get() * 70}")
        if self.maza.get() != 0:
